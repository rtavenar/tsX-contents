...
#cetz.canvas(
  ...
  // ConvNets
  content((T / 2, 3), text(oldlink.darken(40%), size: 24pt, [1D Conv.]))
  for t in range(T) {
    circle((t * delta_x, 0),
      radius: radius,
      fill: xi,
  ...
  for t in range(target_t - 1, target_t + 2) {
    line("conv_x_" + str(t) + ".north", "conv_y_" +  str(target_t) + ".south", mark: (end: "stealth"), stroke: black + 2pt)
  }
  ...
  for t in range(T) {
    line("sa_x_" + str(t) + ".north", "sa_y_" +  str(target_t) + ".south", mark: (end: "stealth"), stroke: black + 2pt)
  }
